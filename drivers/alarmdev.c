...
/*devide driver FILE
--------------------------------------------------------------------------------
--!@brief Device Driver do Alarm dev --------------------------------------------------------------------------------                                                                      
--! DESCRIPTION:
--!
--! driver do Alarm Dev para uClinux
--!
--! DETAILS:      
--!
--!- Project Name: alarmdev                       
--!- Module Name: alarmdev.c
--!- Tools: jedit 
--!- Author: Franz Wagner
...
#define ALARMDEV_1 			0x028100B0
#define ALARMDEV_2 			0x028100C0
#define ALARMDEV_3 			0x028100D0
#define ALARMDEV_4 			0x028100E0
#define ALARMDEV_5 			0x028100F0
#define ALARMDEV_6 			0x02810100
#define ALARMDEV_7 			0x02810110
#define ALARMDEV_8 			0x02810120
#define ALARMDEV_LAYER 		0x02810130
#define ALARMDEV_RESET 		0x028100A0
#define DEVICE_SIZE			0x10

#define BUF_SIZE			33 // buffer ASI = 1~32, buffer layer = 33
#define DEVICE_NAME			"alarmdev"
...
#ifdef DEBUG
		printk(KERN_ALERT "ALARMDEV: device busy\n");
#endif
...
#ifdef DEBUG
	printk(KERN_ALERT "ALARMDEV: open\n");
#endif
...
#ifdef DEBUG
	printk(KERN_ALERT "ALARMDEV: release\n");
#endif
...
#ifdef DEBUG
	printk(KERN_ALERT "ALARMDEV: llseek [%d]\n", pointer);
#endif
...
#ifdef DEBUG
			printk(KERN_ALERT "ALARMDEV: alarmdev RESET\n");
#endif
		break;
...
#ifdef DEBUG
	printk(KERN_ALERT "ALARMDEV: ioctl [%u]\n", cmd);
#endif
...
alarm_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	alarm_rd_buf[i] = (reg & 0xFF000000) >> 24;
    alarm_rd_buf[i + 1] = (reg & 0x00FF0000) >> 16;
    alarm_rd_buf[i + 2] = (reg & 0x0000FF00) >> 8;
    alarm_rd_buf[i + 3] = reg & 0x000000FF;
	i += 4;

	reg = IORD(ALARMDEV_2, 0);
	alarm_rd_buf[i] = (reg & 0xFF000000) >> 24;
    alarm_rd_buf[i + 1] = (reg & 0x00FF0000) >> 16;
    alarm_rd_buf[i + 2] = (reg & 0x0000FF00) >> 8;
    alarm_rd_buf[i + 3] = reg & 0x000000FF;
    ...
    reg = IORD(ALARMDEV_LAYER, 0);
    alarm_rd_buf[i] = reg & 0x000000FF;
...
#ifdef DEBUG
		printk(KERN_ALERT "ALARMDEV: read\n");
#endif
...
#ifdef DEBUG
		printk(KERN_ALERT "ALARMDEV: write\n");	
#endif	
		return count;
	}
}
...
int i=0;
	...
	if (!request_region(ALARMDEV_1, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_1);
		return -EIO;
	}
	if (!request_region(ALARMDEV_2, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_2);
		return -EIO;
	}
	if (!request_region(ALARMDEV_3, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_3);
		return -EIO;
	}
	if (!request_region(ALARMDEV_4, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_4);
		return -EIO;
	}
	if (!request_region(ALARMDEV_5, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_5);
		return -EIO;
	}
	if (!request_region(ALARMDEV_6, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_6);
		return -EIO;
	}
	if (!request_region(ALARMDEV_7, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_7);
		return -EIO;
	}
	if (!request_region(ALARMDEV_8, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O port 0x%x is not free.\n", ALARMDEV_8);
		return -EIO;
	}
	if (!request_region(ALARMDEV_RESET, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O (alarmdev RESET) is not free \n");
		return -EIO;
	}
	if (!request_region(ALARMDEV_LAYER, DEVICE_SIZE, DEVICE_NAME)) {
		printk(KERN_ERR "ALARMDEV: I/O Layer is not free.\n");
		return -EIO;
	}	
	...
	/* All Done! */
	printk(KERN_ALERT "ALARMDEV Driver Initialized\n");
	return 0;
}
...
alarm_cleanup(void)
{
	...
	release_region(ALARMDEV_2, DEVICE_SIZE);
	release_region(ALARMDEV_3, DEVICE_SIZE);
	release_region(ALARMDEV_4, DEVICE_SIZE);
	release_region(ALARMDEV_5, DEVICE_SIZE);
	release_region(ALARMDEV_6, DEVICE_SIZE);
	release_region(ALARMDEV_7, DEVICE_SIZE);
	release_region(ALARMDEV_8, DEVICE_SIZE);
	release_region(ALARMDEV_RESET, DEVICE_SIZE);
	release_region(ALARMDEV_LAYER, DEVICE_SIZE);
