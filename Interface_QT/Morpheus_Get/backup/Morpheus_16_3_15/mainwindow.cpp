            ...
            if(filestr.open(QIODevice::ReadOnly)==true)
            {
                ...
                for(int a=0;a<=255;a++)
                {
                    for(int b=0;b<=255;b++)
                    {
                        ...
                        {
                            image12b.setPixel(b,a, image12b.colorTable().at(qCeil(colors.at(cont).toUInt()/3.2)));
            ...
            if(filestr.open(QIODevice::ReadOnly)==true)
            {
                ...
                for(int a=0;a<=255;a++)
                {
                    for(int b=0;b<=255;b++)
                    {
                        ...
                        {
                            image24b.setPixel(b,a, image24b.colorTable().at(qCeil(colors.at(cont).toUInt()/100)));
                        }
                        contagem_anterior=contagem;
                        contagem=contagem+colors.at(cont).toFloat();
                        cont++;
        ...
        if(y>=1 && y<=255)
        {
            ...
            if(Count_L.toInt()==1)
            {
                ...
                for(int a=0;a<1536;a++)
                {
                    ...
                    {
                        ui->Value_input->setText(QString::number(qCeil(colortable.indexOf(image12b.pixel(x,y))*3.3)));
                    }
                }
            }
...
void MainWindow::on_Start_Scan_Button_clicked()
{
    ...
    ui->custom_Plot->graph(0)->rescaleAxes(true);
    ...
    double total_cont=0;
    double total_cont_ant=0;
    QVector<double> x(512), y(512);
    QVector<double> dx(512), dy(512);

    for(int a=Scan_From.toInt(); a>=Scan_To.toInt(); a-=Scan_Step.toInt())
    {
        QString url = "http://10.0.27.67/cgi-bin/dacs.cgi?&threshold_0=";
        url.append(QString::number(a)+"&write");
        nam->get(QNetworkRequest(QUrl(url)));
        //qDebug()<<url;
        QThread::msleep(100);
        ...
        QStringList arguments_pserve;
        process_pserve = Current_Path+"/pserve";
        arguments_pserve<<Count_L<<"1"<<Current_Path+"/scan/"+scan_name+QString::number(a);
        myProcess->start(process_pserve,arguments_pserve);
        myProcess->waitForStarted();
        ...
        QByteArray stream;
        QFile filestr(Current_Path+"/scan/"+scan_name+QString::number(a)+"1.ppm");
        if(filestr.open(QIODevice::ReadOnly)==true)
        {
            QVector<double> xi(1), yi(1);
            stream=filestr.readAll();
            QString bitstr(stream);
            QStringList bit_pixeis = bitstr.split(QRegExp("\\s+"));
            bit_pixeis.removeLast();
            bit_pixeis.removeFirst();
            bit_pixeis.removeFirst();
            bit_pixeis.removeFirst();
            bit_pixeis.removeFirst();
            for(int b=0;b<bit_pixeis.count();b++)
            {
                total_cont_ant=total_cont;
                total_cont=total_cont+bit_pixeis.at(b).toInt();
                y[a]=total_cont;
                if(b>=1)
                {
                    dy[a]=-(total_cont_ant-total_cont);
                    yi[0]=dy[a];
                }
           ...
           dx[a] = a;
           xi[0] = a;
           ui->custom_Plot->graph(0)->addData(xi,yi);
           ui->custom_Plot->graph(0)->rescaleAxes(true);
           ui->custom_Plot->update();
           QCoreApplication::processEvents(QEventLoop::AllEvents);
        }
        //ui->custom_Plot->graph(0)->addData(dx,dy);
