...
void MainWindow::realtimeDataSlot()
{
    if(scan>=0)
    {
        ...
        if (key-lastPointKey > 3) // de 3 em 3 segundos
        {
            ...
            if(Count_L.toInt()==1) // 12 bits
            {
                ...
                if(filestr.open(QIODevice::ReadOnly)==true)
                {
                    ...
                    colortable=ctable.colorPalete1280();
                    image12b.setColorTable(colortable);
                    total_cont_ant=contagem;
                    int cont=0;
                    for(int a=0;a<=255;a++)
                    {
                        for(int b=0;b<=255;b++)
                        {
                            if(qCeil(bit_pixeis.at(cont).toInt()/3.201)>=1280)
                            {
                                image12b.setPixel(b,a, image12b.colorTable().at(1279));
                            }
                            else
                            {
                                image12b.setPixel(b,a, image12b.colorTable().at(qCeil(bit_pixeis.at(cont).toUInt()/3.201)));
                            }

                            contagem_anterior=contagem;
                            contagem=contagem+bit_pixeis.at(cont).toFloat();
                            cont++;
                            if(contagem_anterior!=contagem)
                            {
                                pixeis_lidos++;
            ...
            dcont=(contagem-total_cont_ant);
            ui->Contagem_Input->setText(QString::number(contagem));
            ui->ContagemAnt_Input->setText(QString::number(total_cont_ant));
            ui->diff_input->setText(QString::number(dcont));
...
{
            xtotal+=5;
            ui->custom_Plot->xAxis->setRange(0,xtotal);
            ui->custom_Plot->replot();
