...
QImage image12bb(256,256,QImage::Format_RGB16);
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ui->Tab_Preview->setCurrentIndex(0);
    ...
    ui->Plot_Window_2->setMouseTracking(true);
    ui->Plot_Window_2->scale(2,2);
...
void MainWindow::on_Start_Aquire_clicked()
{
    ...
    if(RCount.toInt()==2)
    {
        arguments_pserve<<Count_L<<NImage<<Save_Path;//arguments_pserve<<"4"<<NImage<<Save_Path;
    }
    else
    {
    ...
    **************************************************************************************************/
    if(NImage.toInt()==1)
    {
        ...
        else if(Count_L.toInt()==1) // 12 bits
        {
            ...
            if(RCount.toInt()==2)
            {
                proc = myProcess->waitForFinished((Time.toFloat()*1000)+2000);
                if(proc==true)
                {
                    QByteArray teste;
                    ...
                    QByteArray teste2;
                    QFile filestr2(Save_Path+"1th1.ppm");
                    if(filestr2.open(QIODevice::ReadOnly)==true)
                    {
                        teste2=filestr2.readAll();
                        QString colorstr2(teste2);
                        QStringList colors2 = colorstr2.split(QRegExp("\\s+"));
                        colors2.removeLast();
                        colors2.removeFirst();
                        colors2.removeFirst();
                        colors2.removeFirst();
                        colors2.removeFirst();
                        QVector<QRgb> colortable2;
                        Lookup_Tables ctable2;
                        colortable2=ctable2.colorPalete1280();
                        image12bb.setColorTable(colortable2);
                        int cont=0;
                        for(int a=0;a<=255;a++)
                        {
                            for(int b=0;b<=255;b++)
                            {
                                if(qCeil(colors2.at(cont).toInt()/3.201)>=1280)
                                {
                                    image12bb.setPixel(b,a, image12bb.colorTable().at(1279));
                                }
                                else
                                {
                                  image12bb.setPixel(b,a, image12bb.colorTable().at(qCeil(colors2.at(cont).toUInt()/3.201)));
                                }
                    ...
                    //image12bb.fill(Qt::black);
                    pixmap = QPixmap::fromImage(image12b,Qt::ColorOnly);
                    pixmap2 = QPixmap::fromImage(image12bb,Qt::ColorOnly);
                }
            }
            else
            {
                proc = myProcess->waitForFinished((Time.toFloat()*1000)+1000);
                if(proc==true)
                {
                    QByteArray teste;
                    QFile filestr(Save_Path+"1.ppm");
        ...
        scene= new QGraphicsScene(ui->Plot_Window);
        scene->addPixmap(pixmap);
        ui->Plot_Window->setScene(scene);
        Graphics_view_zoom* z = new Graphics_view_zoom(ui->Plot_Window);
        z->set_modifiers(Qt::ControlModifier);
        scene2= new QGraphicsScene(ui->Plot_Window_2);
        scene2->addPixmap(pixmap2);
        ui->Plot_Window_2->setScene(scene2);
        Graphics_view_zoom* z2 = new Graphics_view_zoom(ui->Plot_Window_2);
...
void MainWindow::on_Enable_All_Test_clicked()
{
    QStringList Test;
    QString THString;
    QFile File("/home/franz/altera/Interface/cgi_medipix/Interface_QT/Morpheus_Get/build/Equalize/testbit");
...
void MainWindow::ShowContextMenu(const QPoint& pos)
{
    ...
    myMenu.addAction("Mask All");
    myMenu.addAction("Clear Mask");
    myMenu.addAction("Read Mask Bit");
    myMenu.addAction("Read Test Bit");
    myMenu.addAction("Read TH High");
    myMenu.addAction("Read TH Low");
    ...
    if (selectedItem)
    {
        ...
        *   mask all
        **************************************************************************************************/
        else if (selectedItem->text()=="Mask All")
        {
            QString mask_matrix[256][256];
            int conta=0;
            for(int a=0;a<=255;a++)
        ...
        *   Read mask
        **************************************************************************************************/
        else if (selectedItem->text()=="Read Mask Bit")
        {
            QStringList Mask;
            QString THString;
            QFile File("/home/franz/altera/Interface/cgi_medipix/Interface_QT/Morpheus_Get/build/Equalize/maskbit");
        ...
        *   Read Test
        **************************************************************************************************/
        else if (selectedItem->text()=="Read Test Bit")
        {
            QStringList Test;
            QString THString;
            QFile File("/home/franz/altera/Interface/cgi_medipix/Interface_QT/Morpheus_Get/build/Equalize/testbit");
        ...
        *   Read Threshold high
        **************************************************************************************************/
        else if (selectedItem->text()=="Read TH High")
        {
            QStringList Threshold;
            QString THString;
            QFile File("/home/franz/altera/Interface/cgi_medipix/Interface_QT/Morpheus_Get/build/Equalize/ThAdjustmentH");
            ...
            scene->addPixmap(pixmap);
            ui->Plot_Window->setScene(scene);
            ui->textBrowser->setText("!#SUCCESS=Read THH!#");
        }
        /**************************************************************************************************
        *   Read Threshold low
        **************************************************************************************************/
        else if (selectedItem->text()=="Read TH Low")
        {
            QStringList Threshold;
            QString THString;
            QFile File("/home/franz/altera/Interface/cgi_medipix/Interface_QT/Morpheus_Get/build/Equalize/ThAdjustmentL");
...
void MainWindow::realtimeDataSlot()
{
    if(scan>=0)
    {
        ...
        if (key-lastPointKey > wait_time+3) // de 3 em 3 segundos
        {
            ...
            myProcess->waitForStarted();
            nam->get(QNetworkRequest(QUrl(url)));
            bool a = myProcess->waitForFinished((wait_time*1000)+2000);
            //qDebug()<<a <<scan;
            if(a==false)
            {
