...
#define THADJ_L_FILE "ajustes_por_pixel/ThAdjustmentL"
#define EQ_ARQUIVO "equalize.dat"

int main(void)
{
	char matrix[327680];
	char matrix_low[327680];
	char test_bits[65536];
	char mask_bits[65536];
	char threshold_high[196865];
	char threshold_low[196865];
	...
	// lendo arquivo threshold low
	read_file(THADJ_L_FILE,matrix_low,327680);
	cast_threshold_bit(matrix_low,threshold_low);
	memset(matrix_low,'\0',sizeof(matrix_low));
	printf("Load THL\n");
	// conversao ascii binario
	ascii_to_bin(threshold_low, matrix_low);
	memset(threshold_low,'\0',sizeof(threshold_low));
	// gravar stream de saida
	write_out_stream(matrix,matrix_low,mask_bits,test_bits,all_bits);
