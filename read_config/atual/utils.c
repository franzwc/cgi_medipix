...
void cast_threshold_bit(char matrix[327680], char threshold[196865])
{
	...
	/*for(line=0;line<=196863;line++)
	{
		fprintf(stderr,"%c", threshold[line]);
	}*/
	memset(matrix,'\0',sizeof(matrix));
}
...
void ascii_to_bin(char threshold[196865], char matrix[327680])
{
	...
	strcat(matrix_mirror,StrReverse(token2));
	do
	{
		token = strtok(NULL, " ");
		if (token!=NULL)
		{
			...
			token2=StrCastReverse(byte_to_binary(buff));
			//fprintf(stderr,"\ntoken %s buff %d token2 %s",token,buff,token2);
			strcat(matrix_mirror,StrReverse(token2));//StrReverse(token2)
		}
	}while(token!= NULL);
	cont=0;
	for(line=327679;line>=0;line--)
	{
		matrix[line]=matrix_mirror[cont];
		cont++;
	}
	cont=0;
	...
	//for(coluna=0;coluna<=327679;coluna++)
	//{
		//fprintf(stderr,"%c",matrix[coluna]);
	//}
}
